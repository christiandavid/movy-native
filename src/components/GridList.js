...
const renderItem = ({ item, index, columns }) => {
  const posters =
    columns > item.length
  ...
  return (
    <View key={index} style={styles.imageContainer}>
      {posters.map(({ image }, idx) =>
        image ? (
          <MoviePoster key={idx} imageSrc={image} />
        ) : (
...
export function GridList({ posters: rawPosters }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <VirtualizedList
        ...
        renderItem={({ item, index }) => renderItem({ item, index, columns })}
      />
    </SafeAreaView>
  );
}
